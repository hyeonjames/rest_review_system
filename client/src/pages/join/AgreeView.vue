    ...
    <div>
        <header class="header" role="header">
            <div class="container-fluid bg-primary" style="height:auto;">
                <h3 class="text-white">{{type == 'M' ? '회원' : '식당주'}}약관</h3>
            </div>
        </header>
    ...
    본 약관은 2017년 07월 1일부터 적용한다.
            ...
            <div class="row align-items-end">
                <div class="col text-center">
                    <button type="button" class="btn btn-outline-dark" style="height:80px;width:380px" onclick="location.href='#/' "><h2>취소</h2></button>
                </div>
                <div class="col text-center">
                    <button type="button" class="btn btn-primary" style="height:80px;width:380px" v-on:click="approve"><h2>{{type == 'M' ? '회원' : '식당주'}}약관 동의</h2></button>
                </div>
            </div>
        </main>
...
export default {
    ...
    created(){
        if(location.hash.indexOf('/agree/member') >= 0) {
            this.type = 'M';
        } else {
            this.type = 'O'
        }
